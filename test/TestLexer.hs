...
testTokOrExprToNode =
  TestList
    [ "node error" ~: tokOrExprToASTNode [] ~?= ASTNodeError (TokenInfo TokError ""),
      ...
      "node define" ~: tokOrExprToASTNode [T (TokenInfo TokOpenParen "("), T (TokenInfo TokKeywordMutable "define"), A (ASTNodeSymbol "foo"), A (ASTNodeInteger 123), T (TokenInfo TokCloseParen ")")] ~?= ASTNodeMutable (ASTNodeSymbol "foo") (ASTNodeArray [(ASTNodeInteger 123)]),
