...
testStrToAST = TestList [
    ...
    "declare function with one parameter" ~: strToAST "(define foo (a) (1))" ~?= ASTNodeDefine (ASTNodeSymbol "foo") (Valid (ASTNodeParamList [ASTNodeSymbol "a"])) [(ASTNodeInteger 1)],
    "call function foo with one argument" ~: strToAST "(foo 1)" ~?= ASTNodeFunctionCall "foo" [ASTNodeInteger 1]]
