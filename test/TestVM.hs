...
testExecSyscallWrapper =
  TestList
    [ "returns an Invalid ValidState Context for an Invalid input"
      ...
      "sets the exit flag to True for a Valid input with SCExit"
        ~: let x = Valid newContext {exit = True, registers = Registers (Map.fromList [(EAX, 1), (EBX, 0), (ECX, 0), (EDX, 0), (ESI, 0), (EDI, 0), (EBP, 0), (ESP, 0)])}
            in execSyscallWrapper x ~?= Invalid "exit",
